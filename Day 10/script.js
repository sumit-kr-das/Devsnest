...
function flipCard() {
    if (cardsShowing) return;
    if (this === flippedCard) return;
    if (this.cardMatched === true) return;
    console.log(this.id);
    ...
    } else flippedCard = this;

    if (noOfCards === totalPoints * 2) onGameFinished();

    statusDiv.innerHTML = "";
    statusDiv.append(
        `Total Moves ${cardFlipsCount}, Total Points ${totalPoints}`
    );
}
...
for (let i = 0; i < noOfCards; i++) {
    ...
    cardBack.style.backgroundImage =
        "url(images/" + imageMap["id" + (i + 1)] + ".svg)";
